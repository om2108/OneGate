    ...
    <div className="bg-white rounded-2xl shadow-sm hover:shadow-md transition overflow-hidden">
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between gap-3 mt-4">
          {item.joinable && <button className="px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700">Join</button>}
          <button onClick={() => onDetails(item)} className="px-4 py-2 bg-slate-100 text-sm rounded-lg hover:bg-slate-200">View Details</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full">
        ...
        <div className="flex justify-end mt-6">
          <button onClick={onClose} className="px-4 py-2 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700">Close</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {upcoming.map((ev) => <EventCard key={ev.id} item={ev} onDetails={openDetails} />)}
      </div>

      <h2 className="text-2xl font-semibold text-slate-900 mt-12">Past Events</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {past.map((ev) => <EventCard key={ev.id} item={ev} onDetails={openDetails} />)}
      </div>

      <EventModal open={modalOpen} event={activeEvent} onClose={() => setModalOpen(false)} />
    </div>
  );
}
