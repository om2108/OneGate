...
const ApprovedItem = ({ requester, visitor, time }) => {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-green-200 transition">
      <div className="flex-1">
        <div className="text-xs font-semibold text-green-600 uppercase tracking-wider">{requester}</div>
      ...
      <div className="flex flex-col items-start sm:items-end">
        ...
        <span className="text-gray-400 text-xs mt-1">Entered at {time}</span>
      </div>

      <div className="flex gap-2 w-full sm:w-auto">
        <button 
          onClick={() => alert(`Viewing details for ${visitor}`)}
        ...
        <button 
          onClick={() => alert(`${visitor} has checked out.`)}
...
const ApprovedVisitors = () => {
  ...
  const approvedList = visitorData.filter(item => item.status === "Approved");
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Visitors currently inside the premises</p>
          </div>
        </div>
